...
Dieses Skript basiert auf dem Vorlesungsstoff der Vorlesung Mathematische Methoden. Es besteht keine Garantie für Fehlerfreiheit, der Inhalt ist nicht vollständig und ist für die Prüfung angepasst. Falls euch Fehler auffallen, schreibt gerne eine Email an rjahnke@ethz.ch.

\begin{importantbox}[title=Konventionen]
Es gilt $\ii^2=-1$, und positiv orientiert bedeutet gegen den Uhrzeigersinn.
Für die Fourier-Transformation wird die symmetrische Moodle-Konvention verwendet:
\[
...
\]
Der Hauptwert ist eindeutig. Bei Produkten werden Beträge
multipliziert und Argumente addiert; bei Quotienten werden Beträge dividiert und
Argumente subtrahiert.

\begin{kochrezeptbox}[title={Polarform, Potenzen und Wurzeln}]
\begin{enumerate}
\item Bestimme $r=|z|$ und einen Winkel $\varphi$ im richtigen Quadranten.
...
\sin(\ii z)=\ii\sinh z.
\]
Alle vier Funktionen sind ganz. Anders als auf $\R$ sind $\sin z$ und $\cos z$ auf
$\C$ unbeschränkt.
...
\item Die Funktion
\[
f:\C\to\R,\qquad z\longmapsto|\sin z|
\]
ist beschränkt, das heißt, es existiert ein $M\in\R$, sodass
$|\sin z|\le M$ für alle $z\in\C$ gilt.
\[
\square\ \text{Wahr}
\qquad\qquad
\square\ \text{Falsch}
...
\begin{beispielbox}[title={Vorgerechnetes Prüfungsbeispiel: Aufgabe 3 (2024) -- Hauptwert des Logarithmus}]
\textbf{Aufgabe.} Berechnen Sie
\[
\Log\!\left(\frac{\ee^3}{\sqrt2}
+\ii\frac{\ee^3}{\sqrt2}\right).
\]

\textbf{Lösung.} Die Zahl liegt im ersten Quadranten. Ihr Betrag und ihr
Hauptargument sind
\[
\left|\frac{\ee^3}{\sqrt2}
+\ii\frac{\ee^3}{\sqrt2}\right|=\ee^3,
\qquad
\Arg\!\left(\frac{\ee^3}{\sqrt2}
+\ii\frac{\ee^3}{\sqrt2}\right)=\frac{\pi}{4}.
\]
Also
\[
\frac{\ee^3}{\sqrt2}
...
Komplexe Differenzierbarkeit ist stärker als reelle Differenzierbarkeit
$\R^2\to\R^2$. Reell differenzierbar in $z_0$ bedeutet: Es gibt eine
$\R$-lineare Abbildung $A:\C\to\C$ mit
\[
f(z_0+h)=f(z_0)+A(h)+o(|h|)\qquad(h\to0).
\]
Ist $f=u+\ii v$ und sind die ersten partiellen Ableitungen in einer Umgebung
stetig, dann ist $f$ reell differenzierbar; $A$ wird durch die Jacobi-Matrix
\[
D_{\R}f=
\begin{pmatrix}
u_x&u_y\\
...
\end{pmatrix}
\]
beschrieben. Komplexe Differenzierbarkeit liegt genau dann vor, wenn diese
$\R$-lineare Abbildung zusätzlich eine Multiplikation mit einer komplexen Zahl ist.
Schreibe nun
\[
...
\item Bei stückweise definierten Funktionen zuerst Stetigkeit an der Naht prüfen.
\end{itemize}
\end{pruefungsbox}
...
\end{itemize}
Dabei wird durchgehend die symmetrische Fourier-Normierung verwendet.
\end{pruefungsbox}
...
\item Periodische Fortsetzung gedanklich zeichnen; Sprünge an den Rändern markieren.
...
\begin{mcbox}[title=MC-Training: gewöhnliches Integral und Cauchy-Hauptwert]
Sei $f:\R\to\R$ so, dass
\[
\PV\int_{-\infty}^{\infty}f(t)\dd t
\]
existiert. Entscheiden Sie jeweils, ob die Zusatzannahme garantiert, dass auch
das gewöhnliche uneigentliche Integral existiert und
\[
...
\square\ \text{Richtig}\qquad\qquad\square\ \text{Falsch}
\]
\item Das gewöhnliche Integral
$\int_{-\infty}^{\infty}f(t)\dd t$ existiert.
\[
\square\ \text{Richtig}\qquad\qquad\square\ \text{Falsch}
\]
\item $f$ ist gerade.
...
\end{cases}
\]
In der symmetrischen Konvention gilt
\[
\boxed{
\widehat{f_a}(\omega)
...
Sinusintegrale.
\item Bei Quotienten für $\omega=0$ den Grenzwert separat angeben.
\item Ergebnis auf Parität, Konjugationssymmetrie und Abklingen für
$|\omega|\to\infty$ prüfen.
\end{enumerate}
\end{kochrezeptbox}
...
\begin{enumerate}
\item Den Ausdruck mit bekannten Transformationspaaren der symmetrischen
Normierung vergleichen.
...
\item Produkte im Frequenzraum bei Bedarf als Faltungen im Zeitraum interpretieren.
\end{enumerate}
\end{kochrezeptbox}
...
\]
Für $t\geq0$ folgt dies direkt aus
\[
\int_0^t\ee^{-a\tau}\dd\tau
=\frac{1-\ee^{-at}}a;
\]
für $t<0$ verschwindet die Faltung wegen der beiden Heaviside-Faktoren.
\end{satzbox}
...
\item Den Ausdruck als bekanntes Transformationspaar oder Produkt erkennen.
...
\begin{mcbox}[title=MC-Training: Wachstumskoeffizienten und Rechenregeln]
Es seien $f_1,f_2:\R\to\C$ zwei Originalfunktionen. Sei $\sigma_1$ ein
Wachstumskoeffizient von $f_1$ und $\sigma_2$ ein Wachstumskoeffizient von
$f_2$. Entscheiden Sie für jede Aussage, ob sie wahr oder falsch ist.
\begin{enumerate}
\item Falls $\sigma_1\neq\sigma_2$, so gilt
$\mathcal L[f_1]\neq\mathcal L[f_2]$.
\[
...
\]
\item Der Wachstumskoeffizient von $f_1+f_2$ ist $\sigma_1+\sigma_2$.
\[
\square\ \text{Wahr}\qquad\qquad\square\ \text{Falsch}
\]
\item Für $f(t)=H(t)t\cos t$ gilt
\[
\mathcal L[f](s)
=-\frac{1-s^2}{(s^2+1)^2}.
\]
(Hier bezeichnet $H$ die Heaviside-Funktion.)
\[
\square\ \text{Wahr}\qquad\qquad\square\ \text{Falsch}
\]
\item Der Wachstumskoeffizient von $f_1f_2$ ist $\sigma_1\sigma_2$.
\[
\square\ \text{Wahr}\qquad\qquad\square\ \text{Falsch}
\]
\end{enumerate}
...
\]
Ein Dirac-Impuls modelliert eine konzentrierte Anregung und kann bei einer
Differentialgleichung einen Sprung in der Lösung oder einer Ableitung erzeugen.

\subsection{Faltung und Laplace-Transformation}
Für kausale Funktionen ist die Faltung
\[
(f*g)(t)=\int_0^t f(\tau)g(t-\tau)\dd\tau.
\]
Es gilt ohne zusätzlichen Normierungsfaktor
\[
...
\]
wobei $c$ rechts von allen Singularitäten liegt. In Prüfungsrechnungen verwendet man
meist Tabellen, Verschiebungsregeln, quadratische Ergänzung und
...
Anteil zerlegen.
\item Tabelle rückwärts lesen und Verschiebungen $\ee^{-as}$ als
Zeitverzögerungen erkennen.
\end{enumerate}
\end{kochrezeptbox}
...
\item Anfangswerte und nach Möglichkeit die Differentialgleichung kontrollieren.
\end{enumerate}
\end{kochrezeptbox}
...
\]
Der rechtsseitige Wert der Lösung ist $f(0^+)=4$. Der Dirac-Impuls bei
$t=0$ erzeugt also einen Sprung der Größe $1$ gegenüber dem angegebenen
Vorimpulswert $f(0)=3$.
\end{beispielbox}
...
$\mathcal L[f']=sF-f(0)$.
\item Spezielle Terme systematisch ersetzen:
...
Singularität trifft. Zusätzlich sei $\wind(\gamma,w)=0$ für alle $w\notin U$
(insbesondere genügt es, dass der von $\gamma$ umschlossene Bereich in $U$ liegt).
Dann
\[
\int_\gamma f(z)\dd z
=2\pi\ii\sum_{k=1}^{n}
...
\item Bei einem reellen Ausgangsintegral kontrollieren, dass das Endergebnis reell ist.
\end{enumerate}
\end{kochrezeptbox}
...
\]
sofern die genannten Voraussetzungen erfüllt sind.
...
\item Prüfe, dass der Bogenbeitrag verschwindet.
...
\]
mit zusätzlichen symmetrischen Aussparungen an reellen Polen. Er kann existieren,
obwohl das gewöhnliche uneigentliche Integral nicht existiert.
...
\]
Die Formel hängt von der gewählten Einrückung ab; sicherer ist es, den Beitrag des
kleinen Bogens jedes Mal mit Orientierung herzuleiten.
...
bestimmen.
\item Das Rechteck so wählen, dass nur wenige Pole darin liegen, und deren
Residuen berechnen.
...
Kante beachten.
\item Den Residuensatz anwenden, die Kantenbeiträge einsetzen und
$R\to\infty$ nehmen.
\item Den verbleibenden komplexen Vorfaktor vereinfachen und prüfen, dass das
Ergebnis zum reellen Ausgangsintegral passt.
\end{enumerate}
\end{kochrezeptbox}

Entscheidend ist nicht die Form der Kontur, sondern dass ihre Kanten zur Struktur
des Integranden passen.
...
\textbf{Vorprüfung: Konvergenz des reellen Integrals.}
Für $x\to+\infty$ verhält sich der Integrand wie
$\ee^{-(1-a)x}$, für $x\to-\infty$ wie $\ee^{ax}$. Wegen
$0<a<1$ ist das gewöhnliche uneigentliche Integral absolut konvergent.
...
\begin{itemize}
\item Zuerst den Integraltyp erkennen: Bei einer vollen Periode führt meist
$z=\ee^{\ii t}$ zum Einheitskreis; bei einem uneigentlichen Fourier-artigen
Integral entscheidet der Abfall des Exponentialfaktors zwischen Halbkreis und
Rechteck.
...
\item Orientierung konsequent verfolgen: Die obere Rechteckkante und eine untere
Halbkreiskontur liefern typischerweise ein zusätzliches Minuszeichen.
\end{itemize}
\end{pruefungsbox}
}
...
\item Berechnung komplexer Integrale mit der Cauchyschen Integralformel, wobei die
Kurve vorgegeben ist und ihre Lage beziehungsweise Orientierung ausgewertet werden
muss;
...
\item Das Integral in lokale, einmal positiv umlaufene Standardintegrale
zerlegen.
\item Jeden relevanten Term auf
$h(z)/(z-z_0)^{n+1}$ mit bei $z_0$ holomorphem Zähler $h$ bringen.
\item Für $n=0$ die Cauchy-Formel und für $n\geq1$ die Ableitungsformel
verwenden.
\item Die lokalen Integrale mit ihren Windungszahlen multiplizieren und
addieren.
\end{enumerate}
\end{kochrezeptbox}
